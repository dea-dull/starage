...
export const handler = async (event) => {
  ...
  if (!token) {
    return {
      statusCode: 302,
      headers: { Location: `${process.env.FRONTEND_URL}/verify?reason=missing_token` }
    };
  }
  ...
  try {
    ...
    if (Item.used) {
      return { statusCode: 302, headers: { Location: `${process.env.FRONTEND_URL}/verify?reason=already_used` } };
    }

    if (Item.expiresAt < Math.floor(Date.now() / 1000)) {
      return { statusCode: 302, headers: { Location: `${process.env.FRONTEND_URL}/verify?reason=expired` } };
    }
    ...
    return {
      statusCode: 302,
      headers: { Location: `${process.env.FRONTEND_URL}/verify?reason=success` }
    };
  } catch (error) {
    console.error('Verify error:', error);
    return {
      statusCode: 302,
      headers: { Location: `${process.env.FRONTEND_URL}/verify-error?reason=error` }
    };
  }
};
